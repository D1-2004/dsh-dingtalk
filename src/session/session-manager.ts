/**
 * SessionManager — 管理钉钉 peer → dsh Agent 的映射和生命周期
 *
 * sessionKey 格式: `dingtalk:${clientId}:${scope}:${peerId}`
 *   - direct: peerId = senderStaffId
 *   - group:  peerId = openConversationId
 *
...
export class SessionManager {
  ...
  private sessionKey(scope: ChatScope, peerId: string): string {
    return `dingtalk:${this.config.clientId}:${scope}:${peerId}`;
  }
  ...
  private async composePreset(presetId?: string): Promise<PresetComposition> {
    ...
    try {
      ...
      this.logger.warn(
        `im-dingtalk: preset ${presetId ?? '(default)'} unavailable: ${err instanceof Error ? err.message : String(err)} — using host composition`,
      );
      return {};
    }
  }
